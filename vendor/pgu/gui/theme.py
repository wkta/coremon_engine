...
from coremon_main import PygameBridge
...
#                 s = w._theme_paint_bkgr.convert()
    ...
    def event(self,w,func):
        def theme_event(e):
            ...
            if e.type == PygameBridge.MOUSEBUTTONUP or e.type == PygameBridge.MOUSEBUTTONDOWN:
                ...
                sub = pygame.event.Event(e.type,{
                    'button':e.button,
                    'pos':(e.pos[0]-rect.x,e.pos[1]-rect.y)})
            elif e.type == PygameBridge.MOUSEMOTION:
                sub = pygame.event.Event(e.type,{
                    'buttons':e.buttons,
                    'pos':(e.pos[0]-rect.x,e.pos[1]-rect.y),
                    'rel':e.rel})
